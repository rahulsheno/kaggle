...
def train(name, seed, nch, mk, N=24000, epochs=6, batch=64):
...
def sim_eval(a, c, n=1600, seed=1234):
    """NRMSE by sigma bin on simulated archive-noise crops."""
    rng = np.random.default_rng(seed)
    idx = rng.integers(0, len(XALL), n)
    from _utils import warp_batch
    ...
    Zz, _, SIG = corrupt_batch(X0, rng, t_mix=MIX)
    ...
    print('SIM overall NRMSE %.4f' % e.mean())
    for lo, hi in ((0, 0.35), (0.35, 0.5), (0.5, 0.65), (0.65, 0.8), (0.8, 1.3)):
        m = (sig >= lo) & (sig < hi)
        if m.sum():
            print('  sig [%.2f,%.2f): n=%d NRMSE %.3f' % (lo, hi, m.sum(), e[m].mean()))
    return e.mean()


if __name__ == '__main__':
    from _dev15_cls_final import DenNet
    da_old = DenNet(2); da_old.load_state_dict(torch.load('_den_a.pt')); da_old.eval()
    dc_old = DenNet(4); dc_old.load_state_dict(torch.load('_den_c.pt')); dc_old.eval()
    print('--- OLD ensemble on sim archive ---', flush=True)
    old = sim_eval(da_old, dc_old)
    a = train('a3', 7, 2, make_features_batch)
    c = train('c3', 11, 4, make_features_C_batch)
    print('--- NEW ensemble on sim archive ---', flush=True)
    new = sim_eval(a, c)
    ...
    with torch.no_grad():
        ...
        RC = c(torch.from_numpy(FC)).numpy()[:, 0]
    R = np.clip((RA + RC) / 2, 0, 1)
    print('NEW ensemble calib NRMSE %.4f' % nrmse(R, CC))
    print('summary: old sim %.4f -> new sim %.4f' % (old, new), flush=True)
