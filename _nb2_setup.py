...
# Vectorized versions of the GIVEN chain, plus a per-crop noise-level estimator.
import numpy as np
...
def corrupt_batch(S, rng, t_mix=MIX):
    """The given chain, vectorized: x0 -> optics -> diffusion read noise -> bit rot -> stored range."""
    S = np.asarray(S)
    B = len(S)
...
def sigma_mad_raw_batch(Zz):
    ...
    ZD = np.where(bad, med, Zz)                      # desalt isolated bit-rot extremes
...
BM, BT = build_sigma_correction(X_TRAIN, rng)
print("sigma correction table:", len(BM), "bins; est 0.3 -> %.3f, est 0.45 -> %.3f"
      % (np.interp(0.3, BM, BT), np.interp(0.45, BM, BT)))

def sigma_estimate_batch(Zz):
    est, ZD = sigma_mad_raw_batch(Zz)
    return np.interp(est, BM, BT, left=0.0, right=BT[-1]), ZD

def make_features_A_batch(Zz):
    """view A: [desalted z, sigma]"""
...
def make_features_C_batch(Zz):
    """view C: [normalized z, raw z, sigma, clipmask]"""
    sig, ZD = sigma_estimate_batch(Zz)
    clip0 = (np.asarray(Zz) <= -0.349) | (np.asarray(Zz) >= 1.349)
